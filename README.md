...
- **Asks both archives for the full text.** Europe PMC does not serve `fullTextXML` for author manuscripts (the NIHMS and EMS deposits), so those used to arrive as an abstract even though the whole paper was in PubMed Central. Review now tries Europe PMC, then NCBI, and takes whichever copy actually carries an article body. Over a 63-paper reference list this took full text from 23 papers to 32.
- Falls back to the PubMed abstract (via Europe PMC, then NCBI E-utilities) when neither archive has the article, and says so plainly.
- **Re-fetch** on any saved paper pulls it again from its own identifier and keeps the better copy, so a paper saved before an extraction fix — or before its full text was reachable — can be brought up to date without being re-imported. It never trades a saved full text for an abstract.
