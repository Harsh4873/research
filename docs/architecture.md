...
- Account gate: sets live under `recall_users/{uid}`, so sync is single-account by construction — another Google account would be another, empty library, and the rules deny it anyway. `checkSyncAccount` (pure, in `sync-core.ts`) compares the signed-in address to `OWNER_EMAIL` **before** any listener is opened; a mismatch parks the engine in an error state that names both addresses and offers *Switch account*, and clears `this.user` so nothing is ever read, written, or pushed under the wrong uid. A `permission-denied` that survives that check means the project is still running a ruleset without Recall's block, and the message says so along with the command that fixes it (`npm run deploy:rules`).
